...
namespace ClayAudioEngine
{
    public class ElaborationUnitGlyphInstance : ElaborationUnitGlyph , IXMLAble
    {
        ...
        public void write(XmlTextWriter writer)
        {
            ...
            //Maybe there is nothing to serialize
            /*
            for(int inputIndex=0;inputIndex<inputGlyphs.Count();inputIndex++)
            {
                inputGlyphs[inputIndex].write(writer);
            }
            for (int outputIndex = 0; outputIndex < outputGlyphs.Count(); outputIndex++)
            {
                outputGlyphs[outputIndex].write(writer);
            }
            */
