...
/*
void Oscillator::updateAudioSamples(EAG_SAMPLE_TYPE *pSamplesBuffer,int numsamples)
{
	int i,j;
	...
	case e_Sine:
		{
			for(j=0;j<MIDIChannelMessage::NumMIDINotes;++j)
			{
				if( m_pVoices[j]->m_bActive )
					for(i=0;i<numsamples;i++)
					{
						EAG_SAMPLE_TYPE envLevel = m_pVoices[j]->m_pEnvelope->getUpdatedLevel();
						if(m_pVoices[j]->m_bFinalRelease)
						{
							if(m_pVoices[j]->m_pEnvelope->releaseCompleted())
							{
								m_pVoices[j]->Dispose();
								break;
							}
						}
						pSamplesBuffer[i] += m_Amplitude * envLevel * m_pAmplitudeInBuffer[i] * sin( m_pVoices[j]->m_TimeAccumulator * 2.0 * 3.14159265358979323846 / m_pVoices[j]->m_Period );
						m_pVoices[j]->m_TimeAccumulator += m_SamplingTime;
						//if( m_pVoices[j]->m_TimeAccumulator >= m_pVoices[j]->m_Period )
						//{
						//	m_pVoices[j]->m_TimeAccumulator = m_pVoices[j]->m_TimeAccumulator - m_pVoices[j]->m_Period;
						//}
					}
			}
			break;
			...
			for(j=0;j<MIDIChannelMessage::NumMIDINotes;++j)
			{
				if( m_pVoices[j]->m_bActive )
					for(int i=0;i<numsamples;i++)
					{
						...
						if( m_pVoices[j]->m_TimeAccumulator >= (m_pVoices[j]->m_Period / 2.0) )
							pSamplesBuffer[i] += m_Amplitude * m_pAmplitudeInBuffer[i] * (+1.0) * envLevel;
						else
							pSamplesBuffer[i] += m_Amplitude * m_pAmplitudeInBuffer[i] * (-1.0) * envLevel;

						//printf("%d",pSamplesBuffer[i]);
			...
			for(j=0;j<MIDIChannelMessage::NumMIDINotes;++j)
			{
				if( m_pVoices[j]->m_bActive )
					for(int i=0;i<numsamples;i++)
					{
						...
						EAG_SAMPLE_TYPE increment = 1.0 / (this->m_pModuleServices->getEngineSettings()->samplingFrequence / m_pVoices[j]->m_Period);
						EAG_SAMPLE_TYPE level = m_pVoices[j]->m_TimeAccumulator * increment;
						int iLevel = (int) level;
						level = level -iLevel;
						pSamplesBuffer[i] += m_Amplitude * m_pAmplitudeInBuffer[i] * envLevel * level;
						//if( m_pVoices[j]->m_TimeAccumulator >= (m_pVoices[j]->m_Period / 2.0) )
						//	pSamplesBuffer[i] += m_pAmplitudeInBuffer[i] * (+1.0) * envLevel;
						//else
						//	pSamplesBuffer[i] += m_pAmplitudeInBuffer[i] * (-1.0) * envLevel;
...
void Oscillator::receiveMIDIMessage(MIDIChannelMessage& midimsg)
{
	switch( midimsg.Kind )
	{
	case MIDIChannelMessage::e_NoteOn:
		{
			...
			{
				m_pVoices[midimsg.data.NoteMessage.Note]->Deactivate();
			}
			else
			{
				//This is a true NoteOn
				m_pVoices[midimsg.data.NoteMessage.Note]->Activate(midimsg.data.NoteMessage.Frequency);
			}
			break;
		}
...
/*
void Oscillator::Activate(double initialfreq)
{
	assert(!m_bActive);
...
void Oscillator::Deactivate(void)
{
	assert(m_bActive);
	m_bActive = false;
}
...
/*
int Oscillator::setProperty(int propertyIndex, const char* value)
{
	double bufAmp = 0.0;
	if(sscanf_s(value,"%f",bufAmp)==1)
	{
		m_Amplitude = bufAmp;
		return ElaborationUnit::C_PropertyGetSucceess;
	}
	else
		return ElaborationUnit::C_PropertyGetFailure;
}

int Oscillator::getProperty(int propertyIndex, char* value, int bufferSize)
{
	sprintf_s(value,bufferSize,"%f",m_Amplitude);
	return 0;
}
*/